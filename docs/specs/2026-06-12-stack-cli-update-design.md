...
- **P5 env-line cleanup — OPTIONAL/REMAINING**: dropping the now-redundant hand-declared `PHOENIX_VERSION`/`AGENTGATEWAY_VERSION` env lines (auto-seeded from `images.default`) is cosmetic; harmless duplicates left in place to avoid churn.
