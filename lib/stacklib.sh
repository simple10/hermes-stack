...
# Root-validity sentinel: both tracked files MUST be present.
#  - lib/stacklib.sh: we just sourced it; confirms the dirname/.. dance
#    landed on a dir that has the lib/ we came from.
#  - .stack.defaults.env: tracked, hermes-stack-specific (drives `just
#    setup`). Catches mis-resolution into another repo that happens to
#    also have a lib/stacklib.sh.
# docker-compose.yaml is NOT a sentinel — it's generated by
# stack_render_compose (gitignored, may be absent on a fresh checkout);
# dc() rebuilds it on demand.
if [ ! -f "$STACK_ROOT/lib/stacklib.sh" ] || [ ! -f "$STACK_ROOT/.stack.defaults.env" ]; then
...
# stack_render_compose — write $STACK_ROOT/docker-compose.yaml from
# docker-compose.example.yaml (tracked header) + an `include:` block listing
# services/<svc>/compose.yaml for every <svc> in stack_profiles (= COMPOSE_PROFILES
# closed under SERVICE_REQUIRES). Idempotent + atomic (.tmp + mv).
#
# Why generated: Compose parses + interpolates every `include:`-d file
# regardless of profile, so a disabled service's empty ${VAR} references
# trigger warnings on string fields and HARD parse errors on typed fields
# (cpus:, mem_limit:). Filtering the include list to only enabled services
# keeps Compose's view consistent with COMPOSE_PROFILES.
#
# Called by: `just enable`/`disable`/`build`/`start`, and dc() as a safety net.
stack_render_compose() {
  local out="$STACK_ROOT/docker-compose.yaml"
  local example="$STACK_ROOT/docker-compose.example.yaml"
  local enabled svc d
  # space-pad both sides so word-match in `case` is unambiguous
  enabled=" $(stack_profiles | tr ',' ' ') "
  {
    [ -f "$example" ] && cat "$example"
    echo
    echo "# >>> GENERATED by stack_render_compose. Do not edit; rebuilt by"
    echo "# >>> just enable/disable/build/start (and on demand by dc() if missing)."
    echo "# >>> Source of truth: docker-compose.example.yaml + COMPOSE_PROFILES."
    echo "include:"
    for d in "$STACK_ROOT"/services/*/; do
      svc="$(basename "$d")"
      [ -f "$d/compose.yaml" ] || continue
      case "$enabled" in *" $svc "*) echo "  - services/$svc/compose.yaml" ;; esac
    done
    echo "# <<< GENERATED"
...
dc() {
  ...
  proj="$(stack_project)"
  prof="$(stack_profiles)"
  # Auto-heal: regenerate docker-compose.yaml if missing (fresh checkout,
  # or someone deleted it). enable/disable/build/start regenerate explicitly;
  # this is the safety net for any other code path that calls dc() first.
  [ -f "$STACK_ROOT/docker-compose.yaml" ] || stack_render_compose
