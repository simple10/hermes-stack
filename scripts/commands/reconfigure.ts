...
const renderables = (svc: string): Renderable[] => {
  ...
  switch (svc) {
    case 'litellm':
      return [{ template: T('config.yaml.template'), output: O('config.runtime.yaml') }]
    case 'cliproxyapi':
      return [{ template: T('config.yaml.template'), output: O('config.runtime.yaml') }]
    case 'honcho':
      return [{ template: T('config.toml.template'), output: O('config.runtime.toml') }]
    case 'searxng':
      return [{ template: T('settings.yml.template'), output: O('settings.yml') }]
    default:
      return []
  }
}
...
export const runReconfigure = async (args: readonly string[]): Promise<void> => {
  ...
  if (list.length === 0)
    die(`reconfigure: '${svc}' has no known templates (or templates aren't wired here yet)`)
