...
export const runSetup = async (): Promise<void> => {
  ...
  const project = await p.text({
    ...
    validate: (v) => {
      ...
      if (!/^[a-z0-9][a-z0-9_-]*$/.test(v)) return "lowercase letters/digits/_/- only; must start with alnum";
      return undefined;
    },
  });
  ...
  if (typeof orVal === "string" && orVal.length > 0) envUpsert(STACK_ENV, "OPENROUTER_API_KEY", orVal);
  ...
  if (enabled.has("cliproxyapi")) {
    ...
    if (genIfMissing("CLIPROXY_MANAGEMENT_KEY", "", 32)) p.log.success("generated CLIPROXY_MANAGEMENT_KEY");
  }
  if (enabled.has("hermes-workspace")) {
    if (genIfMissing("HERMES_WORKSPACE_PASSWORD", "", 32)) p.log.success("generated HERMES_WORKSPACE_PASSWORD");
  }
  ...
  if (enabled.has("hermes")) {
    ...
    if (group.allowedUsers !== undefined) stackUpsert("HERMES_TELEGRAM_ALLOWED_USERS", group.allowedUsers);
    if (group.homeChannel !== undefined) stackUpsert("HERMES_TELEGRAM_HOME_CHANNEL", group.homeChannel);
  ...
  p.outro(pc.green("Setup complete.") + " Next: ./stack-cli build && ./stack-cli start (not yet ported — for now use `just build && just start`).");
};
