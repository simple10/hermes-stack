...
export const stackProject = (): string =>
  envGet(STACK_ENV, "COMPOSE_PROJECT_NAME") || "aitools";
...
export const stackBackends = (seed?: string): string[] => {
  const profiles = stackProfiles(seed).split(",").map((x) => x.trim()).filter(Boolean);
  const out: string[] = [];
...
export const stackVmName = (svc: string): string =>
  `${stackProject()}-${svc}`;
