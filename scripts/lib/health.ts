...
export const getStackHealth = async (): Promise<StackHealth> => {
  ...
  const enabledServices = new Set(
    stackProfiles().split(",").map((x) => x.trim()).filter(Boolean),
  );

  // -- containers ----------------------------------------------------------
  let dockerLines: string[] = [];
  try {
    const r = await $`docker ps -a --filter ${`label=com.docker.compose.project=${project}`} --format ${'{{.Names}}|{{.Status}}|{{.Image}}|{{.Label "com.docker.compose.service"}}'}`;
    dockerLines = r.stdout.split("\n").map((s) => s.trim()).filter(Boolean);
  } catch { /* docker missing */ }
  ...
  for (const line of dockerLines) {
    ...
    byService.set(service, {
      service, containerName: name, image, status, run, health,
      enabled: enabledServices.has(service.replace(/-(api|deriver|ui)$/, () => "")) || enabledServices.has(service),
    });
  }
  ...
  for (const svc of enabledServices) {
    if (!byService.has(svc)) {
      byService.set(svc, {
        service: svc, containerName: "", image: "", status: "(no container)",
        run: "missing", health: "none", enabled: true,
      });
    }
  }
  ...
  } catch { /* orb missing or not running */ }
...
export const summarize = (h: StackHealth): {
  ...
  return {
    totalServices, upServices, healthyServices,
