...
const discoverImages = (): ImageDecl[] => {
  ...
  for (const entry of readdirSync(SERVICES_DIR)) {
    ...
    } catch { /* not a service dir / unreadable */ }
  }
  return out;
};
...
export const resolveAllImages = async (): Promise<void> => {
  ...
  for (const dir of readdirSync(SERVICES_DIR)) {
    ...
    try {
      ...
      const repos = (body.match(/^[A-Z][A-Z0-9_]*_IMAGE_REPO=/gm) ?? []).map((s) => s.replace("_IMAGE_REPO=", ""));
      const defs = new Set((body.match(/^[A-Z][A-Z0-9_]*_IMAGE_DEFAULT=/gm) ?? []).map((s) => s.replace("_IMAGE_DEFAULT=", "")));
      for (const name of repos) if (!defs.has(name)) die(`${f}: ${name}_IMAGE_REPO without matching ${name}_IMAGE_DEFAULT`);
    } catch { /* unreadable */ }
  }
