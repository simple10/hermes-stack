...
const stateBadge = (run: RunState, health: HealthState): string => {
  const runWord =
    run === "running"    ? pc.green("running")    :
    run === "exited"     ? pc.dim("exited")       :
    run === "missing"    ? pc.dim("missing")      :
    run === "restarting" ? pc.yellow("restarting"):
                           pc.dim(run);
  if (run !== "running") return runWord;
  const h =
    health === "healthy"   ? pc.green("healthy")   :
    health === "unhealthy" ? pc.red("unhealthy")   :
    health === "starting"  ? pc.yellow("starting") : "";
...
export const formatMachineLines = (machines: MachineHealth[]): string[] => {
  ...
  return machines.map((m) => {
    ...
    const state =
      m.run === "running" ? pc.green("running") :
      m.run === "stopped" ? pc.dim("stopped")   :
                            pc.red("missing");
