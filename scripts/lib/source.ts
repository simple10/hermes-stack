...
export const stackSource = async (
  ...
  if (existsSync(resolve(srcDir, '.git'))) {
    ...
    if (originUrl && originUrl !== repo) {
      die(
        `stackSource(${svc}): ${srcDir} origin '${originUrl}' != expected '${repo}' (re-clone manually if intended)`,
      )
    }
  }
  ...
  if (existsSync(resolve(srcDir, '.git'))) {
    let head = ''
    try {
      head = (await $`git -C ${srcDir} rev-parse HEAD`).stdout.trim()
    } catch {
      /* ignore */
    }
    ...
    if (!lockReq && head && /^[0-9a-f]{40}$/.test(requested) && head === requested) {
      ...
      log(
        `stackSource(${svc}): adopt existing checkout @ ${head.slice(0, 12)} (rebuild flag set for fresh project)`,
      )
  ...
  if (!sha) {
    try {
      await $`git -C ${srcDir} fetch origin ${requested}`
    } catch {
      /* swallow */
    }
    sha =
