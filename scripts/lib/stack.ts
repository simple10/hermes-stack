...
export const blockToggle = (svc: string, target: "enabled" | "disabled"): void => {
  ...
  for (const line of lines) {
    if (startEn.test(line)) { inBlock = true; state = "enabled"; }
    else if (startDis.test(line)) { inBlock = true; state = "disabled"; }
    let next = line;
    if (inBlock) {
    ...
    out.push(next);
    if (endEn.test(line) || endDis.test(line)) { inBlock = false; state = "missing"; }
  }
...
export const blockSync = (svc: string, schema: string): void => {
  ...
  for (const line of lines) {
    if (startRe.test(line)) { inBlock = true; continue; }
    if (endRe.test(line)) { inBlock = false; continue; }
    if (inBlock) {
...
export const stackUpsert = (key: string, value: string): void => {
  ...
  let written = false;
  for (const line of lines) {
    if (startRe.test(line)) { inOwner = true; inAny = true; out.push(line); continue; }
    if (inOwner && endRe.test(line)) {
      if (!written) out.push(`${key}=${value}`);
      inOwner = false; inAny = false; written = true;
      out.push(line);
      continue;
    }
    if (otherStartRe.test(line)) { inAny = true; out.push(line); continue; }
    if (inAny && otherEndRe.test(line)) { inAny = false; out.push(line); continue; }
    if (inOwner && keyRe.test(line)) { out.push(`${key}=${value}`); written = true; continue; }
...
const isEnabled = (svc: string): boolean => {
  for (const k of ["COMPOSE_PROFILES", "STACK_MACHINES"] as const) {
    ...
    if (v && v.split(",").map((x) => x.trim()).includes(svc)) return true;
  }
  return false;
};
