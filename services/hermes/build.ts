...
const resolveCtx = (): Ctx => {
  ...
  return { vm, project, remoteUser, hermesVirtualKey, mountEnabled, macHermes, vmHermes, orbMountSpec: `${macHermes}:${vmHermes}` };
};

const hermesWrite = (ctx: Ctx, rel: string, content: string): void => {
  if (!ctx.mountEnabled) {
    warn(`skip ~/.hermes/${rel} (HERMES_MOUNT_ENABLED=false). apply manually with: orb -m ${ctx.vm} bash -lc 'umask 077 && mkdir -p ~/.hermes/$(dirname ${JSON.stringify(rel)}) && cat > ~/.hermes/${rel}'`);
    return;
  }
...
const ensureOrbMachine = async (ctx: Ctx): Promise<void> => {
  log(`1. orb create ubuntu ${ctx.vm} (--user ${ctx.remoteUser} --isolated --isolate-network; reuse if exists)`);
  if (await orbMachineExists(ctx.vm)) {
    log(`machine ${ctx.vm} exists — reusing (REMOTE_USER=${ctx.remoteUser} must match the unix user inside the VM)`);
...
const hardenVm = async (ctx: Ctx): Promise<void> => {
  ...
  try {
    await orbExec(ctx.vm, 'snap list cups >/dev/null 2>&1 && { echo "removing cups snap"; sudo snap remove cups; } || echo "cups snap not installed — skipping"', { stdio: "inherit" });
  } catch { /* ok */ }
};
...
const installHermes = async (ctx: Ctx): Promise<void> => {
  ...
  await orbExec(ctx.vm,
    ...
    { stdio: "inherit" });
  await orbExec(ctx.vm, `sudo ln -sf /home/${ctx.remoteUser}/.local/bin/hermes /usr/local/bin/hermes`);
};
  ...
  switch (mem) {
    ...
    case "agentmemory": {
      ...
      if (ctx.mountEnabled) {
        ...
        warn("agentmemory config.yaml merge skipped (mount disabled). Apply manually inside the VM.");
      }
...
const patchModelBlock = (ctx: Ctx): void => {
  log(`5. patch config.yaml model: block (litellm.${ctx.project}.orb.local; key via stdin, never argv)`);
  const hm = stackGet("HERMES_MODEL") || "cliproxy/gpt-5.5";
  const tpl = readFileSync(resolve(D, "config/config.yaml.model.tmpl"), "utf8");
  const modelBlock = subst(tpl, { STACK_PROJECT: ctx.project, HERMES_MODEL: hm, HERMES_VIRTUAL_KEY: ctx.hermesVirtualKey })
...
const installUnits = async (ctx: Ctx): Promise<void> => {
  ...
  for (const unit of ["hermes-dashboard", "hermes-gateway", "hermes-logtail"]) {
    const tpl = readFileSync(resolve(D, `systemd/${unit}.service`), "utf8")
      .replace(/__REMOTE_USER__/g, ctx.remoteUser);
...
const applyGatewayGate = async (ctx: Ctx): Promise<void> => {
  log("7. HERMES_GATEWAY_ALLOW_ACCESS gate (drop-in survives `hermes gateway restart --system` rewrites)");
  ...
  if (allow) {
    ...
    if (!apiKey) {
      die("HERMES_GATEWAY_ALLOW_ACCESS=true but HERMES_GATEWAY_API_KEY is empty. Run 'stack-cli setup' to mint it.");
    }
    ...
    await orbExecWithStdin(ctx.vm, `sudo tee ${dropIn} >/dev/null && sudo chmod 600 ${dropIn}`, conf);
    ...
    log(`gateway: gate OPEN — drop-in installed; HERMES_GATEWAY_URL=http://${ctx.vm}.orb.local:8642`);
  } else {
    ...
    if (existsSync(gen)) {
      ...
      for (const k of ["HERMES_GATEWAY_URL", "HERMES_DASHBOARD_URL"]) {
        ...
        if (re.test(next)) { next = next.replace(re, ""); removed = true; }
      }
      if (next !== body) writeFileSync(gen, next);
    }
    log(`gateway: gate CLOSED — ${removed ? "removed drop-in + stale HERMES_*_URL" : "loopback-only (default)"}`);
  }
};
...
const materializeManagedEnv = (ctx: Ctx, managed: string[]): void => {
  ...
  log(`~/.hermes/.env managed block rewritten (${managed.length} keys; user lines outside markers preserved)`);
};
