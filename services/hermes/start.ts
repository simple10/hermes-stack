...
export default async function start(): Promise<void> {
  ...
  const active = await orbExec(
    vm,
    'systemctl is-active hermes-dashboard hermes-gateway hermes-logtail | tr "\\n" " "',
  )
  log(`services: ${active}`)
  try {
    const out = await orbExec(
      vm,
      `curl -sS -m6 http://honcho-api.${project}.orb.local:8000/health || true`,
    )
