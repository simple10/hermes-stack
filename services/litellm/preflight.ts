...
// into .stack/litellm/.generated.env.
//
...
// .stack/.env (top-level, owned by `./stack-cli setup`).
//
...
export default async function preflight(): Promise<void> {
  ...
  if (!masterKey) die('LITELLM_MASTER_KEY empty in .stack/.env (run: stack-cli setup)')
  const aliases = (stackGet('LITELLM_VIRTKEYS') || '')
    .split(',')
    .map((x) => x.trim())
    .filter(Boolean)
  if (aliases.length === 0) die('LITELLM_VIRTKEYS empty in .stack/.env (run: stack-cli setup)')
