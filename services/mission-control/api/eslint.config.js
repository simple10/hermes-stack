          ...
          patterns: [
            {
              ...
              message: 'schemas/* must be browser-safe; no drizzle.',
            },
            {
              group: ['better-auth', 'better-auth/*', '@better-auth/*'],
              message: 'schemas/* must be browser-safe; no better-auth.',
            },
            {
              group: ['cloudflare:*'],
              message: 'schemas/* must be browser-safe; no Worker bindings.',
            },
            { group: ['node:*'], message: 'schemas/* must be browser-safe; no Node built-ins.' },
            {
              group: ['../db/*', '../auth/*', '../routes/*'],
              message: 'schemas/* must not depend on server modules.',
            },
          ],
        },
      ],
          ...
          zones: [
            {
              target: './src/schemas',
              from: './src/db',
              message: 'schemas → db is forbidden (browser-safe enforcement).',
            },
            {
              target: './src/schemas',
              from: './src/auth',
              message: 'schemas → auth is forbidden (browser-safe enforcement).',
            },
            {
              target: './src/schemas',
              from: './src/routes',
              message: 'schemas → routes is forbidden (browser-safe enforcement).',
            },
          ],
        },
      ],
