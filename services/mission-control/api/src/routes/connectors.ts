...
export const connectorsRouter = new Hono<{ Bindings: Env; Variables: Variables }>()
...
connectorsRouter.post('/', requireMember('owner', 'admin'), async (c) => {
  try {
    ...
    try {
      ...
      throw new HttpError(
        500,
        'connector.key_mint_failed',
        'Failed to mint connector API key; connector creation rolled back',
      )
    }
...
connectorsRouter.post('/:id/rotate-key', requireMember('owner', 'admin'), async (c) => {
  try {
    ...
    const graceSeconds = parseInt(c.env.KEY_ROTATION_GRACE_SECONDS ?? '', 10) || DEFAULT_GRACE
