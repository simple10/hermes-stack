...
export function tasksRepoTests() {
  describe('tasksRepo', () => {
    ...
    it('countActiveByAgent counts non-terminal tasks', async () => {
      ...
      await db
        .tasks(ctxOwner)
        .insert({ projectId: project.id, title: 'Ready', agentId, status: 'ready' })
      await db
        .tasks(ctxOwner)
        .insert({ projectId: project.id, title: 'In progress', agentId, status: 'in_progress' })
      await db
        .tasks(ctxOwner)
        .insert({ projectId: project.id, title: 'Done', agentId, status: 'completed' })
    ...
    it('throws DuplicateError on idempotency_key collision', async () => {
      ...
      await db
        .tasks(ctxOwner)
        .insert({ projectId: project.id, title: 'First', idempotencyKey: ikey })
      await expect(
