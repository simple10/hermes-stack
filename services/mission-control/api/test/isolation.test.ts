...
async function createAgentInOrgA(suffix: string): Promise<string> {
  const r = await req('POST', '/api/v1/agents', orgAPat, {
    name: `iso-agent-${suffix}`,
    kind: 'hermes',
  })
...
async function createConnectorInOrgA(suffix: string): Promise<string> {
  const r = await req('POST', '/api/v1/connectors', orgAPat, {
    name: `iso-connector-${suffix}`,
    kind: 'notion',
  })
...
async function createProjectInOrg(pat: string, suffix: string): Promise<string> {
  const r = await req('POST', '/api/v1/projects', pat, {
    name: `Iso Project ${suffix}`,
    slug: `iso-proj-${suffix}`,
  })
...
async function createTaskInOrg(pat: string, projectId: string, suffix: string): Promise<string> {
  const r = await req('POST', '/api/v1/tasks', pat, {
...
describe('isolation: /api/v1/connectors', () => {
  ...
  it('PATCH :id — org B gets 404', async () => {
    const r = await req('PATCH', `/api/v1/connectors/${connectorId}`, orgBPat, {
      description: 'hacked',
    })
    expect([403, 404]).toContain(r.status)
  })
...
describe('isolation: /api/v1/projects', () => {
  ...
  it('PATCH :id — org B gets 404', async () => {
    const r = await req('PATCH', `/api/v1/projects/${projectId}`, orgBPat, {
      description: 'hacked',
    })
    expect([403, 404]).toContain(r.status)
  })
...
describe('isolation: /api/v1/tasks', () => {
  ...
  it('org B cannot create a task in org A project', async () => {
    ...
    const r = await req('POST', '/api/v1/tasks', orgBPat, {
...
describe('isolation: /api/v1/tasks/:taskId/comments', () => {
  ...
  beforeAll(async () => {
    ...
    const cRes = await req('POST', `/api/v1/tasks/${taskId}/comments`, orgAPat, {
      body: 'Org A comment',
    })
  ...
  it('POST comment to org A task — org B gets 404', async () => {
    const r = await req('POST', `/api/v1/tasks/${taskId}/comments`, orgBPat, {
      body: 'Hacked comment',
    })
    expect([403, 404]).toContain(r.status)
...
describe('isolation: agent_id filter cross-org', () => {
  it('org A PAT querying ?agent_id=<orgB agent id> returns empty (not org B tasks)', async () => {
    ...
    const orgBAgentRes = await req('POST', '/api/v1/agents', orgBPat, {
      name: `orgb-filter-agent-${Date.now()}`,
      kind: 'hermes',
    })
    if (orgBAgentRes.status !== 201)
      throw new Error(`OrgB agent failed: ${await orgBAgentRes.text()}`)
