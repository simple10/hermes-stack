...
describe('POST /api/v1/agents', () => {
  it('returns 401 when no token', async () => {
    const res = await app.fetch(
      new Request('http://x/api/v1/agents', {
        method: 'POST',
        body: JSON.stringify({ name: 'x', kind: 'hermes' }),
  ...
  it('returns 201 with agent + key on valid create', async () => {
    const res = await req('POST', '/api/v1/agents', pat, {
      name: 'hermes-1',
      kind: 'hermes',
      description: 'First hermes agent',
    })
  ...
  it('member role can create agents', async () => {
    const { pat: memberPat } = await createMemberFixture(
      ...
      orgId,
      'agents-member-create',
      'member',
    )
    const res = await req('POST', '/api/v1/agents', memberPat, {
...
describe('GET /api/v1/agents', () => {
  it('returns 401 without token', async () => {
    const res = await app.fetch(new Request('http://x/api/v1/agents'), TEST_ENV, {
...
describe('PATCH /api/v1/agents/:id', () => {
  ...
  it('returns 200 with updated agent', async () => {
    const res = await req('PATCH', `/api/v1/agents/${agentId}`, pat, {
      name: 'patch-agent-renamed',
  ...
  it('member role can patch agents', async () => {
    ...
    const res = await req('PATCH', `/api/v1/agents/${agentId}`, memberPat, {
...
describe('DELETE /api/v1/agents/:id', () => {
  ...
  it('returns 200 and soft-deletes the agent; subsequent GET returns 404', async () => {
    ...
    const createRes = await req('POST', '/api/v1/agents', pat, {
      name: 'agent-to-delete-full',
      kind: 'hermes',
    })
...
describe('DELETE /api/v1/agents/:id active-task gate', () => {
  it('returns 409 when agent has active tasks', async () => {
    ...
    await (env.DB as D1Database)
      ...
      .bind(projId, orgId, 'gate-proj', `gate-proj-${projId}`, now, now)
      .run()
    ...
    await (env.DB as D1Database)
      ...
      .bind(taskId, orgId, projId, agent.id, 'Active Task', 'in_progress', 0, now, now)
      .run()
    ...
    const body = (await delRes.json()) as {
      error: { code: string; details: { task_ids: string[] } }
    }
...
describe('POST /api/v1/agents — saga compensation (mintApiKey failure)', () => {
  it('returns 500 agent.key_mint_failed and leaves agent soft-deleted', async () => {
    ...
    const row = (await (env.DB as D1Database)
      ...
      .bind(agentName, orgId)
      .first()) as { id: string; deleted_at: number | null; deleted_by_type: string } | null
