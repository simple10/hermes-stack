...
describe('POST /api/v1/projects', () => {
  ...
  it('returns 403 for agent role (agents cannot create projects)', async () => {
    const res = await req('POST', '/api/v1/projects', agentKey, {
      name: 'from-agent',
      slug: 'from-agent',
    })
  ...
  it('returns 201 with member role', async () => {
    const { pat: memberPat } = await createMemberFixture(
      env.DB as D1Database,
      orgId,
      'prj-member-create',
      'member',
    )
    const res = await req('POST', '/api/v1/projects', memberPat, {
      name: 'Member Project',
      slug: 'member-project',
    })
    expect(res.status).toBe(201)
  ...
  it('returns 409 on duplicate slug within org (with existing_project_id)', async () => {
    ...
    const body = (await res.json()) as {
      error: { code: string; details: { existing_project_id: string } }
    }
  ...
  it('allows same slug in different orgs', async () => {
    ...
    const res = await req('POST', '/api/v1/projects', orgBPat, {
      name: 'Org B Project',
      slug: 'my-project',
    })
    expect(res.status).toBe(201)
  ...
  it('emits project.created event', async () => {
    const createRes = await req('POST', '/api/v1/projects', pat, {
      name: 'Event Project',
      slug: 'event-project',
    })
    ...
    const row = (await (env.DB as D1Database)
      ...
      .first()) as { kind: string; resource_id: string } | null
...
describe('GET /api/v1/projects', () => {
  it('returns 401 without token', async () => {
    const res = await app.fetch(new Request('http://x/api/v1/projects'), TEST_ENV, {
...
describe('PATCH /api/v1/projects/:id', () => {
  ...
  it('member role can patch projects', async () => {
    ...
    const res = await req('PATCH', `/api/v1/projects/${projectId}`, memberPat, {
      description: 'by member',
    })
    expect(res.status).toBe(200)
  })
})
...
describe('DELETE /api/v1/projects/:id', () => {
  ...
  it('cascade trigger soft-deletes external_refs on project delete', async () => {
    ...
    await (env.DB as D1Database)
      ...
      .bind(refId, orgId, 'project', project.id, 'notion', 'notion-ws-test', 'ext-casc-1', now, now)
      .run()

    // Verify ref exists.
    const before = (await (env.DB as D1Database)
      .prepare(`SELECT deleted_at FROM external_refs WHERE id = ?`)
      .bind(refId)
  ...
  it('emits project.deleted event on delete', async () => {
    const createRes = await req('POST', '/api/v1/projects', pat, {
      name: 'Event Del Project',
      slug: 'event-del-project',
    })
    expect(createRes.status).toBe(201)
    const { project } = (await createRes.json()) as { project: { id: string } }
