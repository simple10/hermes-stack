...
beforeAll(async () => {
  ...
  if (agentRes.status !== 201)
    throw new Error(`Agent create failed (${agentRes.status}): ${await agentRes.text()}`)
...
describe('POST /api/v1/tasks', () => {
  ...
  it('member role can create tasks', async () => {
    const res = await req('POST', '/api/v1/tasks', memberPat, {
      project_id: projectId,
      title: 'Member Task',
    })
    expect(res.status).toBe(201)
  ...
  it('Layer-2: returns 409 when creating second task with same idempotency_key in same org', async () => {
    ...
    const body = (await res2.json()) as {
      error: { code: string; details: { existing_task_id: string } }
    }
    expect(body.error.code).toBe('idempotency.conflict')
...
describe('GET /api/v1/tasks', () => {
  ...
  beforeAll(async () => {
    const r1 = await req('POST', '/api/v1/tasks', pat, {
      project_id: projectId,
      title: 'List Task 1',
      priority: 10,
    })
  ...
  it('returns 401 without token', async () => {
    const res = await app.fetch(new Request('http://x/api/v1/tasks'), TEST_ENV, {
  ...
  it('cursor pagination returns all tasks exactly once', async () => {
    ...
    const projRes = await req('POST', '/api/v1/projects', freshPat, {
      name: 'Pag Project',
      slug: 'pag-proj',
    })
...
describe('PATCH /api/v1/tasks/:id', () => {
  ...
  describe('state machine transitions', () => {
    it('pending → ready: allowed', async () => {
      ...
      const cr2 = await req('POST', '/api/v1/tasks', pat, {
        project_id: projectId,
        title: 'SM pending task',
      })
    ...
    it('emits task.status_changed event on transition', async () => {
      ...
      const row = (await (env.DB as D1Database)
        ...
        .bind(orgId, t.id)
        .first()) as { kind: string; payload: string } | null
      expect(row).not.toBeNull()
  ...
  describe('agent_id updates', () => {
    ...
    it('auto-promotes pending → ready when agent_id set and no explicit status', async () => {
      ...
      const assigned = (await (env.DB as D1Database)
        .prepare(
      ...
      const statusChanged = (await (env.DB as D1Database)
        .prepare(
          `SELECT kind, payload FROM events WHERE org_id = ? AND resource_id = ? AND kind = 'task.status_changed' ORDER BY id DESC LIMIT 1`,
        )
        .bind(orgId, t.id)
...
describe('DELETE /api/v1/tasks/:id', () => {
  ...
  it('cascade trigger soft-deletes task_comments on task delete', async () => {
    ...
    await (env.DB as D1Database)
      ...
      .bind(cmtId, orgId, t.id, 'usr_test', now, now)
      .run()

    // Verify comment exists.
    const before = (await (env.DB as D1Database)
      .prepare(`SELECT deleted_at FROM task_comments WHERE id = ?`)
      .bind(cmtId)
    ...
    const after = (await (env.DB as D1Database)
      .prepare(`SELECT deleted_at FROM task_comments WHERE id = ?`)
      .bind(cmtId)
      .first()) as { deleted_at: number | null } | null
