...
export const agents = sqliteTable(
  ...
  (t) => ({
    nameUniqueActive: uniqueIndex('agents_name_per_org_active')
      .on(t.orgId, t.name)
      .where(sql`${t.deletedAt} IS NULL`),
    kindIdx: index('agents_org_kind_active')
      .on(t.orgId, t.kind)
      .where(sql`${t.deletedAt} IS NULL`),
  }),
)
...
export async function resolvePoolForOrg(env: Env, orgId: string) {
  ...
  if (!binding)
    throw new HttpError(
      ...
      `Pool binding for ${entry.tenantPoolId} not configured`,
    )
...
export function _clearPoolCache() {
  cache.clear()
}
```
...
describe('pool resolver (single-DB mode)', () => {
  it('returns the same binding for default pool', async () => {
    ...
    await db
      .insert(tenantPools)
      .values({ id: 'default', bindingName: 'DB', createdAt: Date.now() })
      .onConflictDoNothing()
...
export const authMiddleware: MiddlewareHandler = async (c, next) => {
  try {
    ...
    if (session) {
      ...
      if (!orgId)
        throw new HttpError(403, 'auth.no_active_org', 'Session has no active organization')
      ...
      if (!verified.valid || !verified.key)
        throw new HttpError(401, 'auth.invalid_token', verified.error?.message ?? 'Invalid token')
      ...
      if (ptype === 'pat') {
        ...
        if (!m)
          throw new HttpError(
            403,
            'auth.not_member',
            'Key user is not a member of this organization',
          )
        role = m.role as AuthContext['role']
      } else if (ptype === 'agent') {
    ...
    if (!['owner', 'admin', 'member'].includes(ctx.role)) {
      return errorResponse(
        ...
        new HttpError(
          ...
          `Endpoint requires human role; have ${ctx.role}`,
        ),
      )
    }
    if (!allowed.includes(ctx.role as any)) {
      return errorResponse(
        ...
        new HttpError(
          403,
          'auth.role_insufficient',
          `Endpoint requires one of: ${allowed.join(', ')}`,
        ),
      )
    }
    await next()
...
me.get('/', async (c) => {
  ...
  if (ctx.principal.type === 'connector') {
    const cn = await ctx.pool.query.connectors.findFirst({
      where: and(
        eq(connectors.id, ctx.principal.id),
...
app.use(
  ...
  cors({
    origin: (origin) => {
      const allowed =
        (c.env as any).CORS_ALLOWED_ORIGINS?.split(',')
          .map((s: string) => s.trim())
          .filter(Boolean) ?? []
      if ((c.env as any).DB_MODE === 'single' && origin?.startsWith('http://localhost:'))
        return origin
