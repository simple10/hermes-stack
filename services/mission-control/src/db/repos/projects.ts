...
type ProjectInsertInput = Omit<typeof projects.$inferInsert,
  'id' | 'orgId' | 'createdAt' | 'updatedAt' | 'deletedAt' | 'deletedByType' | 'deletedById'>;

type ProjectUpdateInput = Partial<Omit<typeof projects.$inferInsert,
  ...
  return {
    async findById(id: string): Promise<ProjectRow | null> {
      const rows = await ctx.pool.select().from(projects)
        .where(and(scope, eq(projects.id, id))).limit(1);
      return rows[0] ?? null;
    },
    ...
    async list(filter: ProjectListFilter = {}): Promise<ProjectRow[]> {
      ...
      return ctx.pool.select().from(projects)
    ...
    async insert(values: ProjectInsertInput): Promise<ProjectRow> {
      ...
      try {
        ...
        if (isUniqueViolation(e)) {
          ...
          const existing = await ctx.pool.select({ id: projects.id })
            .from(projects)
