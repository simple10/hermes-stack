...
async function insertProject(slug: string): Promise<string> {
  ...
  await (env.DB as D1Database)
    ...
    .bind(id, orgId, `Project ${slug}`, slug, now, now)
    .run()
  return id
}
...
async function insertTask(projectId: string): Promise<string> {
  ...
  await (env.DB as D1Database)
    ...
    .bind(id, orgId, projectId, now, now)
    .run()
  return id
}
...
async function insertComment(taskId: string, index: number = 0): Promise<string> {
  ...
  await (env.DB as D1Database)
    ...
    .bind(id, orgId, taskId, `Comment body ${index}`, now, now)
    .run()
  return id
}
...
async function insertExternalRef(
  ...
  await (env.DB as D1Database)
    ...
    .bind(id, orgId, resourceType, resourceId, `src-${suffix}`, `ext-${suffix}`, now, now)
    .run()
  return id
}
...
async function rawSoftDelete(
  ...
  await (env.DB as D1Database)
    ...
    .bind(deletedAt, deletedByType, deletedById, id)
    .run()
}

/** Fetch deleted_at and deleted_by_type from a task_comments row. */
async function getCommentDelete(
  commentId: string,
): Promise<{ deleted_at: number | null; deleted_by_type: string | null } | null> {
  return (env.DB as D1Database)
    .prepare(`SELECT deleted_at, deleted_by_type FROM task_comments WHERE id = ?`)
    .bind(commentId)
    .first() as Promise<{ deleted_at: number | null; deleted_by_type: string | null } | null>
}

/** Fetch deleted_at and deleted_by_type from an external_refs row. */
async function getRefDelete(
  refId: string,
): Promise<{ deleted_at: number | null; deleted_by_type: string | null } | null> {
  return (env.DB as D1Database)
    .prepare(`SELECT deleted_at, deleted_by_type FROM external_refs WHERE id = ?`)
    .bind(refId)
    .first() as Promise<{ deleted_at: number | null; deleted_by_type: string | null } | null>
}
...
describe('cascade: task soft-delete → comments + external_refs', () => {
  it('cascades to 3 comments and 2 external_refs via Drizzle (app route)', async () => {
    ...
    const projRes = await req('POST', '/api/v1/projects', pat, {
      name: 'Task Cascade Project',
      slug: 'task-cascade-proj',
    })
  ...
  it('does not cascade to unrelated tasks in the same org', async () => {
    const projRes = await req('POST', '/api/v1/projects', pat, {
      name: 'Task No-Touch Project',
      slug: 'task-notouch-proj',
    })
    const { project } = (await projRes.json()) as { project: { id: string } }

    const taskRes1 = await req('POST', '/api/v1/tasks', pat, {
      project_id: project.id,
      title: 'Delete Me',
    })
    const { task: t1 } = (await taskRes1.json()) as { task: { id: string } }

    const taskRes2 = await req('POST', '/api/v1/tasks', pat, {
      project_id: project.id,
      title: 'Keep Me Alive',
    })
...
describe('cascade: project soft-delete → external_refs only (tasks untouched)', () => {
  it('cascades external_refs but leaves project tasks alive', async () => {
    ...
    const taskRow = (await (env.DB as D1Database)
      .prepare(`SELECT deleted_at FROM tasks WHERE id = ?`)
      .bind(taskId)
      .first()) as { deleted_at: number | null } | null
...
describe('cascade: trigger fires even on raw SQL UPDATE (bypasses ORM)', () => {
  it('cascades task comments and external_refs when deleted via raw D1 prepare', async () => {
    ...
    await (env.DB as D1Database)
      ...
      .bind(deletedAt, 'user', 'usr_raw_test', taskId)
      .run()
  ...
  it('raw project delete cascades external_refs but not tasks', async () => {
    ...
    await (env.DB as D1Database)
      ...
      .bind(Date.now(), 'user', 'usr_raw_test', projId)
      .run()
...
describe('cascade: already-deleted children are not re-cascaded', () => {
  it('pre-deleted comment retains its original deleted_at after task delete', async () => {
    const projRes = await req('POST', '/api/v1/projects', pat, {
      name: 'Pre-del Project',
      slug: `pre-del-proj-${Date.now()}`,
    })
    const { project } = (await projRes.json()) as { project: { id: string } }

    const taskRes = await req('POST', '/api/v1/tasks', pat, {
      project_id: project.id,
      title: 'Pre-delete Task',
    })
    const { task } = (await taskRes.json()) as { task: { id: string } }

    // Post a comment via API (simulates normal flow).
    const cRes = await req('POST', `/api/v1/tasks/${task.id}/comments`, pat, {
      body: 'Pre-deleted comment',
    })
    ...
    await (env.DB as D1Database)
      ...
      .bind(earlyTs, 'user', 'usr_early', comment.id)
      .run()
