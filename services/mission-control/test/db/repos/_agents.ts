  ...
  describe('agentsRepo', () => {
    it('insert stamps orgId from ctx', async () => {
      ...
      const agent = await db
        .agents(ctx)
        .insert({ name: 'Hermes', kind: 'hermes', createdByUserId: orgA.userId })
    ...
    it('findById returns the row in the same org', async () => {
      ...
      const agent = await db
        .agents(ctx)
        .insert({ name: 'Worker', kind: 'claude', createdByUserId: orgA.userId })
    ...
    it('findById returns null for a cross-org id', async () => {
      ...
      const agent = await db
        .agents(ctxA)
        .insert({ name: 'SecretAgent', kind: 'hermes', createdByUserId: orgA.userId })
    ...
    it('update with cross-org id returns null', async () => {
      ...
      const agent = await db
        .agents(ctxA)
        .insert({ name: 'OrigName', kind: 'hermes', createdByUserId: orgA.userId })
    ...
    it('softDelete with cross-org id returns null', async () => {
      ...
      const agent = await db
        .agents(ctxA)
        .insert({ name: 'Keepme', kind: 'hermes', createdByUserId: orgA.userId })
    ...
    it('softDelete with system actor override writes deleted_by_type=system', async () => {
      ...
      const agent = await db
        .agents(ctx)
        .insert({ name: 'Compensate', kind: 'hermes', createdByUserId: orgA.userId })
    ...
    it('throws DuplicateError on name collision within the same org', async () => {
      ...
      await db
        .agents(ctx)
        .insert({ name: 'DupAgent', kind: 'hermes', createdByUserId: orgA.userId })
      await expect(
    ...
    it('same name in different org is allowed', async () => {
      ...
      const a = await db
        .agents(ctxA)
        .insert({ name: 'SharedName', kind: 'hermes', createdByUserId: orgA.userId })
      const b = await db
        .agents(ctxB)
        .insert({ name: 'SharedName', kind: 'hermes', createdByUserId: orgB.userId })
