  ...
  describe('connectorsRepo', () => {
    it('insert stamps orgId from ctx', async () => {
      ...
      const connector = await db
        .connectors(ctx)
        .insert({ name: 'Notion', kind: 'notion', createdByUserId: orgA.userId })
    ...
    it('findById returns the row in the same org', async () => {
      ...
      const connector = await db
        .connectors(ctx)
        .insert({ name: 'Linear', kind: 'linear', createdByUserId: orgA.userId })
    ...
    it('findById returns null for a cross-org id', async () => {
      ...
      const connector = await db
        .connectors(ctxA)
        .insert({ name: 'Private', kind: 'custom', createdByUserId: orgA.userId })
    ...
    it('update with cross-org id returns null', async () => {
      ...
      const connector = await db
        .connectors(ctxA)
        .insert({ name: 'OrigConn', kind: 'github', createdByUserId: orgA.userId })
    ...
    it('softDelete with cross-org id returns null', async () => {
      ...
      const connector = await db
        .connectors(ctxA)
        .insert({ name: 'KeepConn', kind: 'notion', createdByUserId: orgA.userId })
    ...
    it('softDelete with system actor override writes deleted_by_type=system', async () => {
      ...
      const connector = await db
        .connectors(ctx)
        .insert({ name: 'CompConn', kind: 'custom', createdByUserId: orgA.userId })
      const deleted = await db
        .connectors(ctx)
        .softDelete(connector.id, { type: 'system', id: null })
    ...
    it("list returns only this org's connectors", async () => {
      ...
      await db
        .connectors(ctxA)
        .insert({ name: 'ConnA', kind: 'notion', createdByUserId: orgA.userId })
      await db
        .connectors(ctxB)
        .insert({ name: 'ConnB', kind: 'linear', createdByUserId: orgB.userId })
    ...
    it('throws DuplicateError on name collision within the same org', async () => {
      ...
      await db
        .connectors(ctx)
        .insert({ name: 'DupConn', kind: 'notion', createdByUserId: orgA.userId })
      await expect(
        db
          .connectors(ctx)
          .insert({ name: 'DupConn', kind: 'linear', createdByUserId: orgA.userId }),
