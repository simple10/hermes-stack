...
describe('cron', () => {
  it('purges events older than retention period and keeps recent ones', async () => {
    ...
    await handleScheduled({ cron: '0 3 * * *' } as ScheduledEvent, env as any, fakeCtx)

    const remaining = await pool.select().from(events).where(eq(events.orgId, ORG))
  ...
  it('purges expired idempotency keys and keeps non-expired ones', async () => {
    ...
    await handleScheduled({ cron: '0 4 * * *' } as ScheduledEvent, env as any, fakeCtx)
  ...
  it('purges expired verification rows and keeps non-expired ones', async () => {
    ...
    await handleScheduled({ cron: '*/15 * * * *' } as ScheduledEvent, env as any, fakeCtx)
  ...
  it('logs a warning for unknown cron expressions', async () => {
    ...
    await expect(
      handleScheduled({ cron: '0 0 1 1 *' } as ScheduledEvent, env as any, fakeCtx),
