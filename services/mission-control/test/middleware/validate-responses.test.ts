...
describe('validateResponses', () => {
  ...
  it('logs a warning when a response drifts from its schema (dev mode)', async () => {
    ...
    app.get('/v1/agents', (c) => c.json({ bogus: true }));

    const res = await app.request('/v1/agents', {}, devEnv);
  ...
  it('does NOT log when the response matches its schema (dev mode)', async () => {
    const app = new Hono();
    app.use('/v1/*', validateResponses);
    app.get('/v1/agents', (c) => c.json({ agents: [], next_cursor: null }));
  ...
  it('no-ops in production mode (skips validation regardless of drift)', async () => {
    const app = new Hono();
    app.use('/v1/*', validateResponses);
    app.get('/v1/agents', (c) => c.json({ bogus: true })); // would drift in dev

    const res = await app.request('/v1/agents', {}, prodEnv);
    expect(res.status).toBe(200);
    expect(warn).not.toHaveBeenCalled();
  });
  ...
  it('skips non-200 responses', async () => {
    const app = new Hono();
    app.use('/v1/*', validateResponses);
    app.get('/v1/agents', (c) => c.json({ error: { code: 'x', message: 'y' } }, 500));
  ...
  it('skips routes not in the schema map', async () => {
    const app = new Hono();
    app.use('/v1/*', validateResponses);
    app.get('/v1/unknown', (c) => c.json({ anything: 'goes' }));

    const res = await app.request('/v1/unknown', {}, devEnv);
    expect(res.status).toBe(200);
    expect(warn).not.toHaveBeenCalled();
  });

  it('matches both /v1/ and /api/v1/ paths (so it survives the Task 1.9 rename)', async () => {
    const app = new Hono();
    app.use('*', validateResponses);
    app.get('/api/v1/agents', (c) => c.json({ also: 'wrong' }));

    const res = await app.request('/api/v1/agents', {}, devEnv);
    expect(res.status).toBe(200);
    expect(warn).toHaveBeenCalledWith(
      '[validate-responses] schema drift',
      expect.objectContaining({ path: '/api/v1/agents' }),
    );
  });
});
