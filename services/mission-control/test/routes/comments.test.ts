/**
 * Integration tests for /api/v1/tasks/:taskId/comments CRUD.
 *
 * Coverage:
...
beforeAll(async () => {
  ...
  const projRes = await req('POST', '/api/v1/projects', pat, { name: 'Comments Project', slug: 'comments-project' });
  ...
  const agentRes = await req('POST', '/api/v1/agents', pat, { name: 'comment-agent-1', kind: 'hermes' });
  ...
  const agentRes2 = await req('POST', '/api/v1/agents', pat, { name: 'comment-agent-2', kind: 'hermes' });
  ...
  const connectorRes = await req('POST', '/api/v1/connectors', pat, { name: 'comment-connector', kind: 'notion' });
  ...
  const orgBProjRes = await req('POST', '/api/v1/projects', orgBPat, { name: 'OrgB Project', slug: 'orgb-project-comments' });
  const orgBProjData = await orgBProjRes.json() as { project: { id: string } };
  orgBProjectId = orgBProjData.project.id;

  const orgBTaskRes = await req('POST', '/api/v1/tasks', orgBPat, { project_id: orgBProjectId, title: 'OrgB Task' });
...
describe('POST /api/v1/tasks/:taskId/comments', () => {
  it('returns 401 without token', async () => {
    const res = await app.fetch(
      new Request(`http://x/api/v1/tasks/${ownedTaskId}/comments`, {
        method: 'POST',
  ...
  it('returns 404 for non-existent task', async () => {
    const res = await req('POST', '/api/v1/tasks/t_doesnotexist/comments', pat, { body: 'hello' });
  ...
  it('returns 400 on missing body field', async () => {
    const res = await req('POST', `/api/v1/tasks/${ownedTaskId}/comments`, pat, {});
  ...
  it('returns 400 on empty body string', async () => {
    const res = await req('POST', `/api/v1/tasks/${ownedTaskId}/comments`, pat, { body: '' });
    expect(res.status).toBe(400);
    const data = await res.json() as { error: { code: string } };
    expect(data.error.code).toBe('comment.validation_error');
  });
  ...
  it('returns 400 on body too long (>10_000 chars)', async () => {
    const res = await req('POST', `/api/v1/tasks/${ownedTaskId}/comments`, pat, { body: 'x'.repeat(10_001) });
    expect(res.status).toBe(400);
    const data = await res.json() as { error: { code: string } };
    expect(data.error.code).toBe('comment.validation_error');
  });
  ...
  it('owner can post a comment → 201', async () => {
    const res = await req('POST', `/api/v1/tasks/${ownedTaskId}/comments`, pat, { body: 'First owner comment' });
  ...
  it('agent can comment on own task → 201 with correct author', async () => {
    const res = await req('POST', `/api/v1/tasks/${ownedTaskId}/comments`, agentKey, { body: 'Agent says hi' });
    expect(res.status).toBe(201);
    // API returns snake_case keys.
  ...
  it('agent 2 gets 403 on task owned by agent 1', async () => {
    const res = await req('POST', `/api/v1/tasks/${ownedTaskId}/comments`, agentKey2, { body: 'Bad agent' });
  ...
  it('connector can comment on any task → 201', async () => {
    const res = await req('POST', `/api/v1/tasks/${ownedTaskId}/comments`, connectorKey, { body: 'Connector comment' });
    expect(res.status).toBe(201);
    // API returns snake_case keys.
  ...
  it('multi-tenant isolation: org A token cannot comment on org B task', async () => {
    const res = await req('POST', `/api/v1/tasks/${orgBTaskId}/comments`, pat, { body: 'Cross-org comment' });
...
describe('GET /api/v1/tasks/:taskId/comments', () => {
  ...
  beforeAll(async () => {
    ...
    for (let i = 0; i < 5; i++) {
      await req('POST', `/api/v1/tasks/${taskForList}/comments`, pat, { body: `Comment ${i + 1}` });
    }
  });

  it('returns 404 for non-existent task', async () => {
    const res = await req('GET', '/api/v1/tasks/t_doesnotexist/comments', pat);
    expect(res.status).toBe(404);
    const data = await res.json() as { error: { code: string } };
    expect(data.error.code).toBe('task.not_found');
  });
  ...
  it('returns 401 without token', async () => {
    const res = await app.fetch(
      new Request(`http://x/api/v1/tasks/${taskForList}/comments`, { method: 'GET' }),
      TEST_ENV,
      { passThroughOnException: () => {} } as any,
    );
    expect(res.status).toBe(401);
  ...
  it('returns comments oldest-first (ASC order)', async () => {
    const res = await req('GET', `/api/v1/tasks/${taskForList}/comments`, pat);
  ...
  it('cursor pagination works correctly', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      await req('POST', `/api/v1/tasks/${paginTaskId}/comments`, pat, { body: `Pagin ${i + 1}` });
    }

    const page1Res = await req('GET', `/api/v1/tasks/${paginTaskId}/comments?limit=2`, pat);
    ...
    const page2Res = await req('GET', `/api/v1/tasks/${paginTaskId}/comments?limit=2&cursor=${encodeURIComponent(page1.next_cursor!)}`, pat);
  ...
  it('agent can list comments on own task', async () => {
    const res = await req('GET', `/api/v1/tasks/${ownedTaskId}/comments`, agentKey);
    expect(res.status).toBe(200);
  ...
  it('agent 2 gets 403 listing comments on task owned by agent 1', async () => {
    const res = await req('GET', `/api/v1/tasks/${ownedTaskId}/comments`, agentKey2);
    expect(res.status).toBe(403);
    const data = await res.json() as { error: { code: string } };
    expect(data.error.code).toBe('comment.forbidden');
  });
  ...
  it('multi-tenant isolation: org A token cannot list org B task comments', async () => {
    ...
    await req('POST', `/api/v1/tasks/${orgBTaskId}/comments`, orgBPat, { body: 'OrgB comment' });

    // Org A token should get 404 when trying to access org B's task.
    const res = await req('GET', `/api/v1/tasks/${orgBTaskId}/comments`, pat);
    expect(res.status).toBe(404);
  });

  it('org B cannot see org A comments even on same task ID if it existed', async () => {
    // Fetch org A's task with org B's PAT → 404.
    const res = await req('GET', `/api/v1/tasks/${taskForList}/comments`, orgBPat);
    expect(res.status).toBe(404);
  });
});
