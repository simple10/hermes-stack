...
beforeAll(async () => {
  ...
  const projRes = await req('POST', '/api/v1/projects', pat, {
    name: 'Comments Project',
    slug: 'comments-project',
  })
  ...
  const agentRes = await req('POST', '/api/v1/agents', pat, {
    name: 'comment-agent-1',
    kind: 'hermes',
  })
  ...
  const agentRes2 = await req('POST', '/api/v1/agents', pat, {
    name: 'comment-agent-2',
    kind: 'hermes',
  })
  ...
  const connectorRes = await req('POST', '/api/v1/connectors', pat, {
    name: 'comment-connector',
    kind: 'notion',
  })
  if (connectorRes.status !== 201)
    throw new Error(`Connector create failed: ${await connectorRes.text()}`)
  ...
  const orgBProjRes = await req('POST', '/api/v1/projects', orgBPat, {
    name: 'OrgB Project',
    slug: 'orgb-project-comments',
  })
...
describe('POST /api/v1/tasks/:taskId/comments', () => {
  ...
  it('agent can comment on own task → 201 with correct author', async () => {
    const res = await req('POST', `/api/v1/tasks/${ownedTaskId}/comments`, agentKey, {
      body: 'Agent says hi',
    })
    expect(res.status).toBe(201)
    // API returns snake_case keys.
  ...
  it('agent 2 gets 403 on task owned by agent 1', async () => {
    const res = await req('POST', `/api/v1/tasks/${ownedTaskId}/comments`, agentKey2, {
      body: 'Bad agent',
    })
  ...
  it('connector can comment on any task → 201', async () => {
    const res = await req('POST', `/api/v1/tasks/${ownedTaskId}/comments`, connectorKey, {
      body: 'Connector comment',
    })
    expect(res.status).toBe(201)
    // API returns snake_case keys.
  ...
  it('multi-tenant isolation: org A token cannot comment on org B task', async () => {
    const res = await req('POST', `/api/v1/tasks/${orgBTaskId}/comments`, pat, {
...
describe('GET /api/v1/tasks/:taskId/comments', () => {
  ...
  it('cursor pagination works correctly', async () => {
    ...
    const page2Res = await req(
      'GET',
      `/api/v1/tasks/${paginTaskId}/comments?limit=2&cursor=${encodeURIComponent(page1.next_cursor!)}`,
      pat,
    )
