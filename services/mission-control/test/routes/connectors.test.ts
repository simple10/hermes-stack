...
describe('POST /api/v1/connectors', () => {
  it('returns 401 without token', async () => {
    const res = await app.fetch(
      new Request('http://x/api/v1/connectors', {
        method: 'POST',
        body: JSON.stringify({ name: 'x', kind: 'notion' }),
  ...
  it('returns 403 for member role (connectors require owner|admin)', async () => {
    ...
    const res = await req('POST', '/api/v1/connectors', memberPat, {
      name: 'notion-1',
      kind: 'notion',
    })
  ...
  it('returns 201 with connector + key on valid create (owner)', async () => {
    const res = await req('POST', '/api/v1/connectors', pat, {
      name: 'notion-1',
      kind: 'notion',
  ...
  it('returns 201 with admin role', async () => {
    const { pat: adminPat } = await createMemberFixture(
      env.DB as D1Database,
      orgId,
      'cnn-admin-create',
      'admin',
    )
    const res = await req('POST', '/api/v1/connectors', adminPat, {
      name: 'linear-admin',
      kind: 'linear',
    })
    expect(res.status).toBe(201)
  ...
  it('allows same name in different orgs', async () => {
    const res = await req('POST', '/api/v1/connectors', orgBPat, {
      name: 'notion-1',
      kind: 'notion',
    })
    expect(res.status).toBe(201)
...
describe('GET /api/v1/connectors', () => {
  it('returns 401 without token', async () => {
    const res = await app.fetch(new Request('http://x/api/v1/connectors'), TEST_ENV, {
...
describe('PATCH /api/v1/connectors/:id', () => {
  ...
  it('returns 200 with updated connector (owner)', async () => {
    const res = await req('PATCH', `/api/v1/connectors/${connectorId}`, pat, {
      name: 'patch-connector-renamed',
      description: 'updated',
    })
    expect(res.status).toBe(200)
  ...
  it('returns 403 when member role tries to patch (connectors require owner|admin)', async () => {
    ...
    const res = await req('PATCH', `/api/v1/connectors/${connectorId}`, memberPat, {
      name: 'hacked',
    })
    expect(res.status).toBe(403)
  })
...
describe('DELETE /api/v1/connectors/:id', () => {
  ...
  it('returns 200 and soft-deletes the connector; subsequent GET returns 404', async () => {
    ...
    const createRes = await req('POST', '/api/v1/connectors', pat, {
      name: 'connector-to-delete-full',
      kind: 'notion',
    })
...
describe('DELETE /api/v1/connectors/:id active external_refs gate', () => {
  it('returns 409 when connector has active external_refs', async () => {
    ...
    await (env.DB as D1Database)
      ...
      .bind(refId, orgId, 'connector', connector.id, 'notion', connector.id, 'ext-abc', now, now)
      .run()

    const delRes = await req('DELETE', `/api/v1/connectors/${connector.id}`, pat)
    expect(delRes.status).toBe(409)
    const body = (await delRes.json()) as {
      error: { code: string; details: { ref_ids: string[] } }
    }
