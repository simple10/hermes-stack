...
beforeAll(async () => {
  ...
  const commentRes = await req('POST', `/api/v1/tasks/${taskId}/comments`, pat, {
    body: 'hello for xref',
  })
  if (commentRes.status !== 201)
    throw new Error(`Comment create failed: ${await commentRes.text()}`)
...
describe('GET /api/v1/external_refs', () => {
  ...
  it('filters by resource_type=task', async () => {
    const res = await req(
      'GET',
      `/api/v1/external_refs?resource_type=task&resource_id=${listTaskId}`,
      pat,
    )
    expect(res.status).toBe(200)
  ...
  it('filters by source_kind=notion', async () => {
    const res = await req(
      'GET',
      `/api/v1/external_refs?source_kind=notion&resource_id=${listTaskId}`,
      pat,
    )
    expect(res.status).toBe(200)
    const data = (await res.json()) as { external_refs: Array<Record<string, unknown>> }
  ...
  it('cursor pagination works', async () => {
    ...
    const page1Res = await req(
      'GET',
      `/api/v1/external_refs?resource_id=${paginTaskId}&limit=2`,
      pat,
    )
    ...
    const page2Res = await req(
      'GET',
      `/api/v1/external_refs?resource_id=${paginTaskId}&limit=2&cursor=${encodeURIComponent(page1.next_cursor!)}`,
      pat,
    )
...
describe('DELETE /api/v1/external_refs/:id', () => {
  ...
  it('owner soft-deletes a ref → 200, ref no longer in GET', async () => {
    ...
    const getRes = await req(
      'GET',
      `/api/v1/external_refs?source_kind=delete-test&source_id=to-delete-ws`,
      pat,
    )
