...
describe('tasks schemas', () => {
  ...
  it('TaskCreateBody rejects malformed idempotency_key', () => {
    ...
    expect(
      TaskCreateBody.safeParse({ project_id: 'prj_a', title: 't', idempotency_key: 'no_colon' })
        .success,
    ).toBe(false)
    // Well-formed → accept.
    expect(
      TaskCreateBody.safeParse({
        project_id: 'prj_a',
        title: 't',
        idempotency_key: 'notion:ws_a:page_b',
      }).success,
    ).toBe(true)
  })
  ...
  it('TaskListQuery accepts both ISO string and ms-epoch for updated_since', () => {
    expect(TaskListQuery.safeParse({ updated_since: '2026-05-24T12:00:00.000Z' }).success).toBe(
      true,
    )
