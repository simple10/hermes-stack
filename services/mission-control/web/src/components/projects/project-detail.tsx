...
export function ProjectDetail({ projectId }: { projectId: string }) {
  ...
  const saveMut = useMutation({
    mutationFn: () => api.projects.patch(projectId, {
      name: name || undefined,
      slug: slug || undefined,
    ...
    <div className="space-y-8 max-w-2xl">
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Input id="slug" defaultValue={p.slug} onChange={(e) => setSlug(e.target.value)} pattern="[a-z0-9-]+" />
        </div>
        <div className="space-y-2">
          <Label htmlFor="desc">Description</Label>
          <Input id="desc" defaultValue={p.description ?? ''} onChange={(e) => setDescription(e.target.value)} />
        </div>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Soft-deletes the project. Associated tasks are unaffected; they keep their project_id reference.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
