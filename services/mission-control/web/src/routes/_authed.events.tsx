    ...
    <div className="space-y-4">
      ...
      <form className="flex flex-wrap gap-2 items-end border rounded p-3" onSubmit={(e) => e.preventDefault()}>
        <div className="space-y-1">
          <Label htmlFor="f-kinds" className="text-xs">Kinds (comma-separated)</Label>
          <Input
            id="f-kinds" defaultValue={filters.kinds ?? ''}
            onChange={(e) =>
        ...
        <div className="space-y-1">
          <Label htmlFor="f-since" className="text-xs">Since (event id)</Label>
          <Input
            id="f-since" type="number" min="0" defaultValue={filters.since ?? ''}
            onChange={(e) =>
              navigate({ search: (s: any) => ({ ...s, since: e.target.value ? parseInt(e.target.value, 10) : undefined }) })
            }
            className="w-32"
          />
        </div>
