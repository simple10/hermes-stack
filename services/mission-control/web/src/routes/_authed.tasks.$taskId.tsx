      ...
      <header className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Project <Link to={`/projects/${task.project_id}` as any} className="underline">{task.project_id}</Link>
            {task.agent_id && (
              <>
                {' '}• Agent <Link to={`/agents/${task.agent_id}` as any} className="underline">{task.agent_id}</Link>
              </>
            )}
          ...
          {externalRefs.length === 0
            ...
            : externalRefs.map((r) => (
              <div key={r.id} className="border rounded p-2 text-sm">
                <code className="text-xs">{r.source_kind}:{r.source_id}</code>
                {' → '}
                {r.external_url
                  ? <a href={r.external_url} target="_blank" rel="noreferrer" className="underline">{r.external_id}</a>
